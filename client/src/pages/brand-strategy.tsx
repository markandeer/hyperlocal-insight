...
import { BrandMission, BrandVision } from "@shared/schema";
...
export default function BrandStrategy() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, content, type }: { id: number; content: string; type: 'mission' | 'vision' }) => {
      const endpoint = type === 'mission' ? `/api/missions/${id}` : `/api/visions/${id}`;
      const res = await apiRequest("PATCH", endpoint, { [type]: content });
      return res.json();
    },
    onSuccess: (_, variables) => {
      const queryKey = variables.type === 'mission' ? "/api/missions" : "/api/visions";
      queryClient.invalidateQueries({ queryKey: [queryKey] });
      setEditingId(null);
      toast({ title: "Updated", description: `${variables.type.charAt(0).toUpperCase() + variables.type.slice(1)} statement updated successfully.` });
    }
  });

  const deleteMutation = useMutation({
    mutationFn: async ({ id, type }: { id: number; type: 'mission' | 'vision' }) => {
      const endpoint = type === 'mission' ? `/api/missions/${id}` : `/api/visions/${id}`;
    ...
    onSuccess: (_, variables) => {
      const queryKey = variables.type === 'mission' ? "/api/missions" : "/api/visions";
      queryClient.invalidateQueries({ queryKey: [queryKey] });
      toast({ title: "Deleted", description: `${variables.type.charAt(0).toUpperCase() + variables.type.slice(1)} statement removed.` });
    }
  });

  const isLoading = isLoadingMissions || isLoadingVisions;
